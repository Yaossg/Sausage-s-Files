...
世界生成是我的世界代码中绕不开的一部分，没有它，玩家将失去几乎一切游戏的载体。

Minecraft发展至今，世界生成的代码却在很长的一段时间里没有发生太大的变化，而1.13正是对这一切进行变革的一个版本。

下面我们将从世界生成的各个方面，逐一探讨其中的奥秘，揭开世界生成神秘复杂的面纱。
...
前面的阶段属于proto（样板）区块，而最后两个属于level（存档）区块，后者已经可以序列化为NBT了。这个两种类型定义在`ChunkStatus.Type`枚举中。另外，前面四个阶段都不会更新高度图，而后面四个都会更新高度图，高度图记录每个`(x, z)`下最高的方块，用于描述地形的起伏升降
...
获取到了生物群系，我们就可以构造地表了，不过前面提到地表的起伏需要高度图来描述，高度图怎么得来，生成随机数？那样生成的地形七上八下支离破碎，~~麻麻赖赖的一点都不圆润~~，为了~~盘它~~确保地形看上去更加平滑真实，就需要用到柏林噪声算法（PerlinNoise），可以最大程度保证不出现极端的高度跳跃。
...
**[提示]** 这一部分很大程度上已经超出了狭义的"世界生成"，作为补充资料放在这里是因为它们有或者曾有与世界生成千丝万缕的联系
...
|`~.GetGrassColor`|草色，新版已经弃用<br>参见前文生物群系栏
|`~.GetFoliageColor`|植物颜色，新版已经弃用<br>参见前文生物群系栏
|`~.GetWaterColor`|水色，新版已经弃用<br>参见前文生物群系栏
...
我这篇文章写的我很艰辛，大量未反混淆的的代码和Mojang奇怪的脑回路使我感到痛苦，但是我还是在最短的时间内肝出这篇文章

感谢MCP对Minecraft源代码的反混淆，如果不是他们，我也不能写出这篇文章

感谢森林蝙蝠审读并提出来让我加入的结构的部分，感谢丢人素学姐和u.s.knowledge对教程提出的宝贵建议

如果你发现本文的任何纰漏，烦请到[github](https://github.com/Yaossg/tutorial)发issue，我会尽快修改，感谢您的指教！

## 相关链接

[本文的github版](https://github.com/Yaossg/tutorial/blob/master/1.13-worldgen.md)
