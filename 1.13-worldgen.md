...
> 特别说明：本文将会持续修订，并会首发于Github，最后更新：2019-4-4
...
中文栏应读作 "最高的 ... 的方块"，如第一个就是 "最高的 非空气 且 非挡光 方块"，括号用于改变优先级。

|英文|中文|
|--|--|
|`LIGHT_BLOCKING`|非空气 且 非挡光
|`MOTION_BLOCKING`|非空气 且 非[允许移动 且 固体]
|`MOTION_BLOCKING_NO_LEAVES`|非空气 且 非树叶（即拥有`minecraft:leaves`标签的方块）<br>且 非[允许移动 且 固体]
|`OCEAN_FLOOR`|非空气 且 非允许移动
|`OCEAN_FLOOR_WG`|非空气 且 非流体
|`WORLD_SURFACE`|非空气
...
在这里，根据不同的函数（`function`），调用了不同的代码，比如生成箱子、生成溺尸。无一例外的，他们往往对附近环境进行的判断和适应，来达到更好的结构生成效果，这是单纯的NBT数据无法做到的。

值得注意的，代码中没忘记清除结构方块，可见结构方块不会因调用函数自己消失。如果函数比较多，`switch`或许是更好的选择。
